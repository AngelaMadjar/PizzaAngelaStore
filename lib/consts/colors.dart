import 'package:flutter/material.dart';

class ColorsConsts{
  static Color black = Color(0xFF000000);
  static Color white = Color(0xFFFFFFFF);
  static Color title = Color(0xDD000000);
  static Color subTitle = Color(0x8A000000);
  static Color backgroundColor = Color(0xFFE0E0E0);

  static Color favColor = Color(0xFFF44336); //red 500
  static Color favBadgeColor = Color(0xFFE57373); //red 300

  static Color cartColor = Color(0xFF5E35B1);
  static Color cartBadgeColor = Color(0xFFBA68C8);

  static Color gradientFStart = Color(0xFFE040FB);
  static Color gradientFEnd = Color(0xFFE1BEE7);
  static Color endColor = Color(0xFFCE93D8);
  static Color purple300 = Color(0xFFBA68C8);
  static Color gradientLEnd = Color(0xFFE91E63); //linear end (vo shopping cart se koristit kaj quantity)
  static Color gradientLStart = Color(0xFF9C27B0); //linear start (vo shopping cart se koristit kaj quantity)
  static Color starterColor = Color(0xFF8E24AA);
  static Color purple800 = Color(0xFF6A1B9A);

}